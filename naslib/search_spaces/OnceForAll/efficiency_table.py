...
class EfficiencyTable:
    def __init__(
        ...
        assert pred_type in ["flops", "latency", "parameters"]
    ...
    @torch.no_grad()
    def measure_single_layer_parameters(self, layer: nn.Module):
        size = 0
    ...
    def build_lut(self, batch_size=1, resolutions=None):
        ...
        if self.pred_type == "parameters":
            np.save(f"local_lut_{self.pred_type}.npy", self.efficiency_dict)
        elif self.pred_type == "flops":
            np.save(f"local_lut_{self.pred_type}.npy", self.efficiency_dict)
        elif self.pred_type == "latency":
            np.save(f"local_lut_{self.pred_type}_{self.device}_bs{self.batch_size}.npy", self.efficiency_dict)
            ...
            if op_type == ResidualBlock:
                ...
                for ks in ks_list:
                    for e in expand_list:
                        ...
                        elif self.pred_type == "parameters":
                            measure_result = self.measure_single_layer_parameters(layer)
